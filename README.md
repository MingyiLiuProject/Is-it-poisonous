...
- 数据库 v2 合并 ASPCA 猫、狗、马的有毒与无毒植物列表，共 1023 条记录
...
第一步解析猫、狗、马列表，合并相同 ASPCA 详情页的状态；第二步通过专业学名匹配 [Wikidata](https://www.wikidata.org/) 与 [GBIF](https://www.gbif.org/) 的中文名，并生成简体中文、别名和拼音索引。自动翻译记录不会伪装成人工审核结果，App 会明确显示复核状态。
...
**Is It Poisonous?** is a SwiftUI iOS prototype for searching plants that may be toxic to cats, dogs, or horses. Its offline catalog supports Chinese, English, scientific-name, pinyin, initials, and typo-tolerant search. It must not be used as a substitute for professional veterinary advice.
